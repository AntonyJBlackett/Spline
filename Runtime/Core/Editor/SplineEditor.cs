...
namespace FantasticSplines
{
    ...
    public class SplineEditor : Editor
    {
        ...
        static Vector3 planePosition;
        static Vector3 planeOffset;
        ...
        public static void FrameCamera(IEditableSpline spline)
        {
            Bounds bounds = nodeSelection.Count > 0 ? GetSelectionBounds( spline ) : GetBounds( spline );
            SceneView.lastActiveSceneView.Frame( bounds, false );
        }

        public static void DeleteNodes(IEditableSpline spline, List<int> nodeIndicies)
        {
            ...
            ClearNodeSelection();
            EditorUtility.SetDirty( spline.Component );
        }

        static void PasteCurve(IEditableSpline spline)
        {
            Debug.Log( "paste" );

            int insertIndex = spline.NodeCount;
        ...
        public static void CopyCurve(IEditableSpline spline)
        {
            clipboard.Clear();
        ...
        public static void StartAddPointMode(IEditableSpline spline, SplineAddNodeMode mode)
        {
            ResetEditMode();
            ...
            if( spline.NodeCount > 0 )
            {
                ...
                {
                    adjoiningIndex = spline.NodeCount - 1;
                }
        ...
        static void InspectorAddPointModeButton( IEditableSpline spline, SplineAddNodeMode mode, GUIStyle normal, GUIStyle selected, string buttonText = "" )
        {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if( foldoutSplineEditor )
            {
                ...
                EditorGUILayout.HelpBox( 
                    ...
                    "Snap to Grid: Caps Lock on/off\n" +
                    "\n" +
                    "Copy/Paste: Select Nodes and " + CrossPlatformControlCommandKey() + " + C / " + CrossPlatformControlCommandKey() + " + V\n" +
                    "Undo/Redo: " + CrossPlatformControlCommandKey() + " + Z / " + CrossPlatformControlCommandKey() + " + Shift + Z" +
                    "\n" +
                    "Frame Camera: Select nodes and press F"
                ...
                InspectorAddPointModeButton( spline, SplineAddNodeMode.Prepend, normal, selected );
                InspectorAddPointModeButton( spline, SplineAddNodeMode.Insert, normal, selected );
                InspectorAddPointModeButton( spline, SplineAddNodeMode.Append, normal, selected );
                ...
                GUILayout.Space( 10 );
                GUILayout.Label( "Set Selected Node(s):" );
                EditorGUILayout.BeginHorizontal();
                {
                    GUILayout.Label( "Tangent Type" );
                    if (GUILayout.Button("Automatic", HasNodeType(spline, nodeSelection, NodeType.Automatic ) ? selected : normal ) )
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Automatic");
                    ...
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Point Nodes" );
                    ...
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Mirror Nodes" );
                    ...
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Aligned Nodes" );
                    ...
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Free Nodes" );
                        SplineEditorTools.SetNodeType( spline, NodeType.Free, nodeSelection );
                    }
                    if( GUILayout.Button( "Simplify" ) )
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Simplify Nodes" );
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    nodeSetPosition = spline.Transform.InverseTransformPoint( nodeSetPosition );
                    nodeSetPosition = EditorGUILayout.Vector3Field( "Position", nodeSetPosition );
                    nodeSetPosition = spline.Transform.TransformPoint( nodeSetPosition );
                    if( EditorGUI.EndChangeCheck() )
                    {
                        ...
                        if( changeX )
                        {
                            Undo.RecordObjects( spline.GetUndoObjects(), "Set Node X Position" );
                            SplineEditorTools.SetNodesXPosition( spline, nodeSelection, nodeSetPosition.x );
                        }

                        if( changeY )
                        {
                            Undo.RecordObjects( spline.GetUndoObjects(), "Set Node Y Position" );
                            SplineEditorTools.SetNodesYPosition( spline, nodeSelection, nodeSetPosition.y );
                        }

                        if( changeZ )
                        {
                            Undo.RecordObjects( spline.GetUndoObjects(), "Set Node Z Position" );
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if( change )
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Set Automatic Tangent Length" );
                ...
                {
                    Undo.RecordObjects( spline.GetUndoObjects(), "Smooth " + workOn );
                    SplineEditorTools.Smooth( spline, nodeIndicies );
                    EditorApplication.QueuePlayerLoopUpdate();
                }
                if( GUILayout.Button( "Flatten " + workOn ) )
                {
                    Undo.RecordObjects( spline.GetUndoObjects(), "Flatten " + workOn );
                    SplineEditorTools.Flatten( spline, nodeIndicies );
                    EditorApplication.QueuePlayerLoopUpdate();
                }
                if( GUILayout.Button( "Snap " + workOn + " To Geometry" ) )
                {
                    Undo.RecordObjects( spline.GetUndoObjects(), "Snap To Geometry " + workOn );
                    SplineEditorTools.SnapToGeometry( spline, nodeIndicies );
                    EditorApplication.QueuePlayerLoopUpdate();
                }
                if( GUILayout.Button( "Simplify " + workOn ) )
                {
                    Undo.RecordObjects( spline.GetUndoObjects(), "Simplify " + workOn );
                    SplineEditorTools.Simplify( spline, nodeIndicies );
                    EditorApplication.QueuePlayerLoopUpdate();
                }
                if( GUILayout.Button( "Reverse " + workOn ) )
                {
                    Undo.RecordObjects( spline.GetUndoObjects(), "Reverse " + workOn );
                ...
                // grid
                {
                    ...
                    if( UniformSnapGrid )
                    {
                        float scalar = EditorGUILayout.FloatField( "Uniform Grid Scale", SnapToGridScaleUniform );
                        SnapToGridScaleUniform = scalar;
                    }
                    else
                    {
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    spline.color = EditorGUILayout.ColorField( "Spline Colour", spline.color );
                    ...
                    spline.alwaysDraw = DoToggleButton( spline.alwaysDraw, "Always Draw" );
                    spline.zTest = DoToggleButton( spline.zTest, "ZTest" );
                ...
                if( foldoutDebug )
                {
                    ...
                    GUILayout.Label( "Node Count: " + spline.NodeCount );
                }
            }
        }

        public static bool EditorActive => !ToolManager.activeToolType.IsSubclassOf( typeof( SplineKeyFrameTool ) );
        ...
        public static float SnapToGridScaleUniform
        {
            get
            {
                return EditorPrefs.GetFloat( "GridSnapScaleX", 1f );
            }
            set
            {
                EditorPrefs.SetFloat( "GridSnapScaleX", value );
            }
        }
        ...
        public static Vector3 SnapToGridScale
        {
            get
            {
                if( UniformSnapGrid )
                {
                    var uniform = SnapToGridScaleUniform;
                    return new Vector3( uniform, uniform, uniform );
                }
                return new Vector3( EditorPrefs.GetFloat( "GridSnapScaleX", 1f ), EditorPrefs.GetFloat( "GridSnapScaleY", 1f ), EditorPrefs.GetFloat( "GridSnapScaleZ", 1f ) );
            }
            set
            {
                EditorPrefs.SetFloat( "GridSnapScaleX", value.x );
                EditorPrefs.SetFloat( "GridSnapScaleY", value.y );
                EditorPrefs.SetFloat( "GridSnapScaleZ", value.z );
            }
        }

        static bool useEvent = false;
        static bool doRepaint = false;

        static void KeyboardInputs(IEditableSpline spline, Event guiEvent)
        {
            if( guiEvent.type == EventType.KeyDown )
            {
                bool shortcutModifier = ShortcutModifer( guiEvent );
                ...
                if( !shortcutModifier )
                {
                    ...
                    {
                        StartAddPointMode(spline, SplineAddNodeMode.Append );
                        guiEvent.Use();
                        return;
                    }
                    if( guiEvent.keyCode == KeyCode.P )
                    {
                        StartAddPointMode(spline, SplineAddNodeMode.Prepend );
                        guiEvent.Use();
                        return;
                    }
                    if( guiEvent.keyCode == KeyCode.I )
                    {
                        StartAddPointMode(spline, SplineAddNodeMode.Insert );
                        guiEvent.Use();
                        return;
                    }
                    ...
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Delete Nodes" );
                        DeleteNodes( spline, nodeSelection );
                        guiEvent.Use();
                        return;
                    }
                ...
                if( shortcutModifier )
                {
                    ...
                    if( nodeSelection.Count > 0 && guiEvent.keyCode == KeyCode.C )
                    {
                        CopyCurve( spline );
                        guiEvent.Use();
                        return;
                    ...
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Cut Nodes" );
                        CopyCurve( spline );
                        DeleteNodes( spline, nodeSelection );
                        guiEvent.Use();
                        return;
                    ...
                    {
                        Undo.RecordObjects( spline.GetUndoObjects(), "Paste Nodes" );
                        PasteCurve( spline );
                        guiEvent.Use();
                        return;
                    }
                ...
                if( nodeSelection.Count > 0 && RaycastModifier( guiEvent ) )
                {
                    guiEvent.Use();
                    return;
                }
        ...
        static void RemoveNode(IEditableSpline spline, int index)
        {
        ...
        public static void DoSceneViewInput( IEditableSpline spline, Event guiEvent )
        {
            useEvent = guiEvent.button != 2 && guiEvent.isMouse;
            ...
            if ( guiEvent.isMouse || guiEvent.isKey
        ...
        public static void DoSceneViewDraw( IEditableSpline spline, Event guiEvent )
        {
            // draw things
        ...
        static Color horizontalGridColour = new Color( .7f, .7f, 1, .2f );
        static void DrawGrid( IEditableSpline spline )
        {
        ...
        static void DrawGrid(Vector3 origin, Vector3 forward, Vector3 right, float scale, int largeGridInterval)
        {
        ...
        static void DrawGridLines(Vector3 origin, Vector3 forward, Vector3 right, float spacing, int halfLength, int halfWidth, Color color)
        {
            Handles.color = color;
        ...
        static void DrawGridLines(Vector3 origin, Vector3 halfLength, Vector3 spacing, int halfCount)
        {
       ...
       public static void ResetEditMode()
        ...
        static void DrawSplinePlaneProjectionLines(IEditableSpline spline)
        {
        ...
        static void DrawSplineLines(IEditableSpline spline)
        {
            for( int i = 0; i < spline.NodeCount; ++i )
            {
                Vector3 point = spline.GetNode( i ).position;
                Vector3 pointOnPlane = MathsUtils.ProjectPointOnPlane( spline.Transform.position, spline.Transform.up, point );
                if( i < spline.NodeCount - 1 )
                {
        ...
        static void DrawBezierSegmentOnPlane(Vector3 planeOrigin, Vector3 planeNormal, SplineNode node1, SplineNode node2)
        {
            ...
            Vector3 tangentFlat1 = MathsUtils.ProjectPointOnPlane( planeOrigin, planeNormal, node1.OutControlPoint );
            Vector3 tangentFlat2 = MathsUtils.ProjectPointOnPlane( planeOrigin, planeNormal, node2.InControlPoint );

            Handles.DrawBezier( pointOnPlane1, pointOnPlane2, tangentFlat1, tangentFlat2, Color.grey, null, 2.5f );
        }

        static void DrawBezierSplineLines(IEditableSpline spline)
        {
            for( int i = 1; i < spline.NodeCount; ++i )
            {
                DrawBezierSegment( spline.GetNode( i - 1 ), spline.GetNode( i ), spline.color );
            }

            if( spline.IsLoop && spline.NodeCount > 1 )
            {
                DrawBezierSegment( spline.GetNode( spline.NodeCount - 1 ), spline.GetNode( 0 ), Color.Lerp( spline.color, Color.red, 0.5f ) );
            }
        }

        static void DrawBezierPlaneProjectedSplineLines(IEditableSpline spline)
        {
            ...
            for( int i = 1; i < spline.NodeCount; ++i )
            {
                DrawBezierSegmentOnPlane( gridMatrix.MultiplyPoint( Vector3.zero ), gridMatrix.MultiplyVector( Vector3.up ).normalized, spline.GetNode( i - 1 ), spline.GetNode( i ) );
            }

            if( spline.IsLoop && spline.NodeCount > 1 )
            {
                DrawBezierSegmentOnPlane( gridMatrix.MultiplyPoint( Vector3.zero ), gridMatrix.MultiplyVector( Vector3.up ).normalized, spline.GetNode( spline.NodeCount - 1 ), spline.GetNode( 0 ) );
            }
        }

        static void DrawSplinePlaneProjectedSplineLines(IEditableSpline spline)
        {
            Matrix4x4 gridMatrix = GetSnapGridLocalToWorldMatrix( spline );
            for( int i = 0; i < spline.NodeCount; ++i )
            {
        ...
        static void DrawNodeControlPoints(IEditableSpline spline, int index)
        {
        ...
        static void DrawSplineSelectionNodeControlPoints(IEditableSpline spline)
        {
            if( !ShowNodeControls )
            {
                return;
        ...
        static void DrawSegmentLengths(IEditableSpline spline)
        {
            if( ShowSplineInfo )
            {
                //SplineDrawer.DrawSegmentLengths(spline);
            }
        }

        static void DrawNodeCoordinates( IEditableSpline spline )
        {
            if( ShowSplineInfo )
            {
                //SplineDrawer.DrawNodeCoordinates(spline, GridSpace == GridSpace.Global ? Space.World : Space.Self);
            }
        }

        static bool IsIndexInRange(IEditableSpline spline, int index)
        {
            return index >= 0 && index < spline.NodeCount;
        }

        static List<int> nodeSelection = new List<int>();
        static void SelectionAddNode(IEditableSpline spline, int index)
        {
            ValidateNodeSelection( spline );
        ...
        static void ValidateNodeSelection(IEditableSpline spline)
        {
            for( int i = nodeSelection.Count - 1; i >= 0; i-- )
            {
        ...
        static List<int> GetSelectedNodeOrAllIndicies(IEditableSpline spline)
        {
        ...
        static List<int> GetNodeIndicies(IEditableSpline spline)
        {
            List<int> nodeIndicies = new List<int>();
            for( int i = 0; i < spline.NodeCount; ++i )
            {
        ...
        static List<int> GetNodeIndiciesSelectedFirst(IEditableSpline spline)
        {
        ...
        static bool IsMouseButtonEvent(Event guiEvent, int button)
        {
        ...
        public struct DetectClickSelectionResult
        {
        ...
        public static DetectClickSelectionResult DetectClickSelection(IEditableSpline spline, Event guiEvent)
        {
        ...
        static bool disableToolDetectionEnabled = false;
        public static bool ShouldDisableTool(IEditableSpline spline, Event guiEvent)
        {
            if (editMode != SplineEditMode.None)
            {
                disableToolDetectionEnabled = false;
                return false;
            }

            if (guiEvent.type == EventType.MouseDown && guiEvent.button == 0)
            {
               disableToolDetectionEnabled = nodeSelection.Count == 0;
            }
            if (guiEvent.type == EventType.MouseDrag && guiEvent.button == 0)
            {
                disableToolDetectionEnabled = false;
            }
            if (disableToolDetectionEnabled && guiEvent.type == EventType.MouseUp && guiEvent.button == 0)
            {
                disableToolDetectionEnabled = false;
                return nodeSelection.Count == 0;
            }

            return false;
        }

        static void DoClickSelection(IEditableSpline spline, Event guiEvent, out bool selectionChanged, out bool clearSelection)
        {
        ...
        static Vector2 mouseDragSelectionStart;
        static bool dragSelectActive = false;
        static int[] dragSelectionStartingSelection;
        static bool DoDragSelection(IEditableSpline spline, Event guiEvent)
        {
            bool clearSelection = IsMouseButtonEvent( guiEvent, 0 ) && !dragSelectActive;
            if( spline.NodeCount > 0 && !dragSelectActive && guiEvent.type == EventType.MouseDown && guiEvent.button == 0)
            {
                dragSelectionStartingSelection = null;
                mouseDragSelectionStart = guiEvent.mousePosition;
                dragSelectActive = true;

                dragSelectionStartingSelection = nodeSelection.ToArray();
            ...
            if( dragSelectActive && guiEvent.button == 0 )
            {
                ...
                ClearNodeSelection();
                if(AddSelectionModifier(guiEvent))
                {
                    if (dragSelectionStartingSelection != null)
                    {
                        foreach (int i in dragSelectionStartingSelection)
                        {
                ...
                if (guiEvent.type == EventType.MouseUp)
                {
                    ...
                    {
                        clearSelection = !AddSelectionModifier( guiEvent );
                    }
        ...
        static bool DoNodeSelection(IEditableSpline spline, Event guiEvent)
        {
            bool clearClick;
        ...
        static AddNodeState addNodeState = AddNodeState.NodePosition;
        static Vector3 addNodePosition;
        static bool canShift = false;
        static void DoAddNode(IEditableSpline spline, Event guiEvent)
        {
            ...
            Selection.activeObject = (spline as Component).gameObject;
            Camera camera = Camera.current;
            Vector3 gridUp = GetSnapGridUp( spline );
            ...
            if( addNodeState == AddNodeState.NodePosition )
            {
                ...
                if ( guiEvent.type == EventType.MouseDown && guiEvent.button == 0 )
                {
                    ...
                    if( result.Detected && !spline.IsLoop )
                    {
                        ...
                        if( addNodeMode == SplineAddNodeMode.Prepend && result.nodeIndex == spline.NodeCount - 1 )
                        {
                            spline.IsLoop = true;
                            guiEvent.Use();
                            editMode = SplineEditMode.None;
            ...
            if( addNodeState == AddNodeState.ControlPosition )
            {
                ...
                if( guiEvent.type == EventType.MouseUp && guiEvent.button == 0 )
                {
                    canShift = false;
                    addNodeState = AddNodeState.NodePosition;
                    Undo.RecordObjects( spline.GetUndoObjects(), "Add Node" );
        ...
        static void DoInsertNode(IEditableSpline spline, Event guiEvent)
        {
            Selection.activeObject = (spline as Component).gameObject;
            ...
            if( guiEvent.type == EventType.MouseDown && guiEvent.button == 0 )
            {
                Undo.RecordObjects( spline.GetUndoObjects(), "Insert Node" );
                spline.InsertNode( closestToRay.percent );
                EditorUtility.SetDirty( spline.Component );
                guiEvent.Use();
            }
        }

        static bool IsMouseOverPoint(Vector3 point, Vector3 mousePosition, float size)
        {
        ...
        public enum MoveControlPointId
        {
            None,
            Control1,
            Control2,
        ...
        static MoveNodeState moveNodeState;
        const float startMovementThreshold = 5;

        static void DetectNodeToMove(IEditableSpline spline, MoveNodeState.MovingModes currentMovingMode, Event guiEvent)
        {
            DetectClickSelectionResult result = DetectClickSelection( spline, guiEvent );
        ...
        static void InitialiseMoveNodeState( IEditableSpline spline, int nodeIndex, MoveControlPointId selectedType, MoveNodeState.MovingModes currentMovingMode )
        {
            ...
            {
                controlPlanePosition = MathsUtils.ProjectPointOnPlane( node.position, spline.Transform.up, node.OutControlPoint );
            ...
            {
                controlPlanePosition = MathsUtils.ProjectPointOnPlane( node.position, spline.Transform.up, node.InControlPoint );
            ...
            if( moveNodeState.Moving )
            {
                planePosition = MathsUtils.ProjectPointOnPlane( spline.Transform.position, spline.Transform.up, node.position );
        ...
        static void DoMoveNode(IEditableSpline spline, Event guiEvent)
        {
            ...
            else if( moveNodeState.Moving && (guiEvent.type == EventType.MouseDrag || guiEvent.type == EventType.MouseUp) && guiEvent.button == 0 )
            {
                ...
                else if( totalMovement.magnitude > 0 )
                {
                    ...
                    Undo.RecordObjects( spline.GetUndoObjects(), "Move Nodes" );
                    if( moveNodeState.MovingNodeControlPoint )
                    {
                        ...
                        if( moveNodeState.ControlPointId == MoveControlPointId.Unknown )
                        {
                            ...
                            if( spline.NodeCount > moveNodeState.NodeIndex + 1 || spline.IsLoop )
                            {
                                int nextIndex = MathsUtils.WrapIndex( moveNodeState.NodeIndex + 1, spline.NodeCount );
                            ...
                            controlPlanePosition = MathsUtils.ProjectPointOnPlane( node.position, spline.Transform.up, worldControl );
        ...
        static bool IsSafeToProjectFromPlane(Camera camera, Vector3 up)
        {
            return Vector3.Dot( camera.transform.forward, up ) < 0.95f;
        }

        static bool TwoDimentionalMode(Camera camera, Vector3 up)
        {
        ...
        static string snapGridModifierString = "toggle capsLock";
        static bool SnapToGridModifier( Event guiEvent )
        {
            return guiEvent.capsLock && !RaycastModifier( guiEvent );
        }

        static bool CrossPlatformControlCommand( Event guiEvent )
        {
#if UNITY_EDITOR_OSX
        ...
        {
            return guiEvent.shift && !CrossPlatformControlCommand(guiEvent) && !guiEvent.alt;
        }
    ...
    public static class SplineEditorTools
    {
        ...
        public static void Flatten(IEditableSpline spline, List<int> nodeIndicies)
        {
            for( int i = 0; i < nodeIndicies.Count; ++i )
            {
                ...
                point.position = MathsUtils.ProjectPointOnPlane( spline.Transform.position, spline.Transform.up, point.position );
                point.InControlPoint = MathsUtils.ProjectPointOnPlane( spline.Transform.position, spline.Transform.up, point.InControlPoint );
                point.OutControlPoint = MathsUtils.ProjectPointOnPlane( spline.Transform.position, spline.Transform.up, point.OutControlPoint );
        ...
        public static void SnapToGeometry( IEditableSpline spline, List<int> nodeIndicies )
        {
            for( int i = 0; i < nodeIndicies.Count; ++i )
            {
                int index = nodeIndicies[i];
                ...
                float castLegnth = 1000;

                Vector3 up = spline.Transform.up * castLegnth * 0.5f;
                if( Physics.Raycast( point.position + up, -up, out RaycastHit hit, castLegnth ) )
                {
                    point.position = hit.point;
                }
                if( Physics.Raycast( point.InControlPoint + up, -up, out RaycastHit hitIn, castLegnth ) )
                {
                    point.InControlPoint = hitIn.point;
                }
                if( Physics.Raycast( point.OutControlPoint + up, -up, out RaycastHit hitOut, castLegnth ) )
                {
                    point.OutControlPoint = hitOut.point;
                }
        ...
        public static void ArcSmooth( IEditableSpline spline )
        {
            ...
            // set control lengths
            for( int i = 0; i < spline.NodeCount - 1; ++i )
            {
                ...
                bool nextNodeIsLastNode = i == spline.NodeCount - 1;
        ...
        // smooths entire spline
        public static void Smooth( IEditableSpline spline )
        {
            if( spline.IsLoop && spline.NodeCount == 2 )
            {
                // special case
            ...
            {
                if( !spline.IsLoop && i == 0 )
                {
                    SmoothStartPoint( spline );
                }
                else if( !spline.IsLoop && i == spline.NodeCount - 1 )
                {
                    SmoothEndPoint( spline );
                }
                else
                {
                    SmoothPoint( spline, i );
                }
            }
        }

        // smooths selected nodes in spline
        public static void Smooth(IEditableSpline spline, List<int> nodeIndicies)
        {
            ...
            if( spline.IsLoop && spline.NodeCount == 2 && nodeIndicies.Count == 2 )
            {
                // special case
                SmoothLoopedStartAndEndPoints( spline );
                return;
            ...
            for (int i = 0; i < nodeIndicies.Count; ++i)
            {
                ...
                else if( !spline.IsLoop && nodeIndicies[i] == spline.NodeCount - 1 )
                {
                    SmoothEndPoint( spline );
                }
                else
        ...
        // smooths node at index in spline
        static void SmoothPoint( IEditableSpline spline, int index )
        {
            ...
            if ( index+1 < spline.NodeCount|| spline.IsLoop )
            {
                Vector3 offset = spline.GetNode(spline.LoopIndex(index + 1)).position - node.position;
                direction -= offset.normalized;
        ...
        // smooths the first node in spline
        ...
        // smooths the last node in spline
        static void SmoothEndPoint(IEditableSpline spline)
        {
            ...
            SplineNode previousNode = spline.GetNode(spline.NodeCount - 2);
            SplineNode endNode = spline.GetNode(spline.NodeCount-1);
            ...
            {
                spline.SetNode( spline.NodeCount - 1, endNode );
                EditorUtility.SetDirty( spline.Component );
            }
        }

        // smooths the first and last node in looped spline
        static void SmoothLoopedStartAndEndPoints( IEditableSpline spline )
        {
            ...
            if( endNode.NodeType == NodeType.Automatic )
            {
                endNode.LocalInControlPoint = -perp * dstBetweenAnchors * endNode.automaticTangentLength;
                endNode.LocalOutControlPoint = perp * dstBetweenAnchors * endNode.automaticTangentLength;
            }
